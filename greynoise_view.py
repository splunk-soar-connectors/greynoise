...
def _parse_data(data, param):
    try:
        ...
        # parse ip sim data
            ...
            if "message" in data[0]:
                ...
                if data[0]["total"] > param["limit"]:
                    res["message"] = (
                        f"Showing first {param['limit']} IPs of {data[0]['total']} IPs that have a "
                        f"similarity score of {param['min_score']}% or above to {param['ip']} "
                    )
                else:
                    res["message"] = (
                        f"Showing {data[0]['total']} IPs that have a similarity score of {param['min_score']}% or above to {param['ip']} "
                    )
        ...
        logger.warning(f"Error in _parse_data: {err!s}")
...
def _get_ctx_result(result, provides):
    try:
        ...
        logger.warning(f"Error in _get_ctx_result: {err!s}")
...
def report(provides, all_app_runs, context):
    try:
        ...
        logger.warning(f"Error in report: {err!s}")
